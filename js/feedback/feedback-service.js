...
class FeedbackService {
  ...
  async submitFeedback(feedback) {
    ...
    if (!feedback.type || !feedback.emotionRating) {
      return Promise.reject(
        new Error('Feedback type and emotion rating are required')
      );
    }
  ...
  getAllFeedback(filters = {}) {
    ...
    return this.feedbackItems.filter(item => {
      ...
      // Filter by rating
      if (filters.minRating && item.emotionRating < filters.minRating)
        return false;
      if (filters.maxRating && item.emotionRating > filters.maxRating)
        return false;
      ...
      if (filters.searchText) {
        ...
        const matchesSearch =
          ...
          (item.user.displayName &&
            item.user.displayName.toLowerCase().includes(searchText));
  ...
  async updateFeedbackStatus(feedbackId, status, adminNotes = null) {
    ...
    return new Promise((resolve, reject) => {
      const feedbackIndex = this.feedbackItems.findIndex(
      ...
      setTimeout(() => {
        ...
        console.log(
          ...
          this.feedbackItems[feedbackIndex]
        );
  ...
  getFeedbackStats() {
    ...
    this.feedbackItems.forEach(item => {
      ...
      // Count by rating
      stats.byRating[item.emotionRating] =
        (stats.byRating[item.emotionRating] || 0) + 1;
