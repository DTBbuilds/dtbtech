...
function setupTimeframeButtons() {
  ...
  timeframeButtons.forEach(button => {
    button.addEventListener('click', () => {
      ...
      const timeframe =
        button.dataset.timeframe || button.textContent.toLowerCase();
...
function setupRealTimeUpdates() {
  ...
  const updateInterval = setInterval(() => {
    ...
    const activeButton = document.querySelector(
      '.analytics-timeframe-btn.bg-blue-500/20'
    );
    const timeframe = activeButton
      ? activeButton.dataset.timeframe || activeButton.textContent.toLowerCase()
      : 'daily';
...
export { initVisualizer, loadAnalyticsData, cleanupVisualizer };
